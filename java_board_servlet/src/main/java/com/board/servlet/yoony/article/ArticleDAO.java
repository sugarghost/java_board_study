...
public interface ArticleDAO {
   ...
   * ArticleDTO#isInsertArticleValid()ArticleDTO#isInsertArticleValid()
   * @since 2023. 02. 17.
   */
   ...
   * 게시글 개수를 조회하는 메소드
   *
   * @param params Map<String, Object> 형태로 담긴 검색 조건               <p>key: searchWord, value: 검색 키워드로
   *                             title, content, writer 세가지 컬럼에 like 연산처리               <p>key:
   *               categoryId, value:               카테고리 ID               <p>key: startDate, value:
   *               날자 검색 시작일로 endDate와 함께 쓰이며 작성일 기간               검색에 사용               <p>key:
   *               endDate, value: 날자 검색 종료일로 startDate와 함께 쓰이며 작성일 기간               검색에 사용
   ...
   *
   * @param articleDTO 검사 대상 articleId, password                   <p>게시글 ID
   ...
   *
   * @param articleDTO 게시글 DTO                   <p>writer, title, content가 update set 대상
   ...
   *
   * @param articleDTO 삭제 대상 articleId, password                   <p>게시글 ID
   *                   <p>게시글 비밀번호 (SHA-256로 암호화된 값)
